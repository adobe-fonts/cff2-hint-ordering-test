...
def build_vf(args, slope=None):
    ...
    if slope:
        ...
        target_dir = ROOT_DIR
        vf_output_name = ROOT_DIR.joinpath( f'{FAMILY_NAME}')
    ...
    if args.hinted:
        ...
        subprocess.call(
            ['python', '-m', 'afdko.otfautohint.splitpsdicts', '-m', hinting_data_file, '-d', designspace_file],
            stdout=STDOUT,
            stderr=STDERR
        )
    ...
    if args.hinted:
        # hint the file
        subprocess.call(
            ['otfautohint', '--no-flex', '-v', '-p', '1', '-k', output_otf],
            stdout=STDOUT,
            stderr=STDERR
        )
